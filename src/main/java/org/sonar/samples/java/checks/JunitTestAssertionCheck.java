...
//import gov.uspto.sonar.java.checks.helpers.AbstractAssertionVisitor;

import static org.apache.commons.lang3.StringUtils.isEmpty;
import static org.sonar.java.checks.helpers.UnitTestUtils.isUnitTest;

@Rule(key = "JCS_4.2.18")
public class JunitTestAssertionCheck extends BaseTreeVisitor implements JavaFileScanner {

    private static final Logger log = LoggerFactory.getLogger(JunitTestAssertionCheck.class);

    @RuleProperty(key = "customAssertionMethods", description = "Comma-separated list of fully qualified method symbols that "
            + "should be considered as assertion methods. The wildcard character can be used at the end of the method name.",
            defaultValue = "")
    ...
    private MethodMatchers getCustomAssertionMethodsMatcher() {
        if (customAssertionMethodsMatcher == null) {
            String[] fullyQualifiedMethodSymbols = customAssertionMethods.isEmpty() ? new String[0]
                    : customAssertionMethods.split(",");
            ...
            for (String fullyQualifiedMethodSymbol : fullyQualifiedMethodSymbols) {
                ...
                if (methodMatcherParts.length == 2 && !isEmpty(methodMatcherParts[0].trim())
                        && !isEmpty(methodMatcherParts[1].trim())) {
                    ...
                    customMethodMatchers.add(MethodMatchers.create().ofSubTypes(methodMatcherParts[0].trim())
                            .name(namePredicate).withAnyParameters().build());
                } else {
                    log.warn("Unable to create a corresponding matcher for custom assertion method, please check the "
                            + "format of the following symbol: '{}'", fullyQualifiedMethodSymbol);
                }
            }
    ...
    private static boolean expectAssertion(MethodTree methodTree) {
        List<SymbolMetadata.AnnotationValue> annotationValues = methodTree.symbol().metadata()
                .valuesForAnnotation("org.junit.Test");
